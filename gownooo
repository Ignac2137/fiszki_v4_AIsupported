...
function renderCard(){
  ...
  let clickFn=(S.confidence||S.flipped)?'flipCard()':'';
  body.innerHTML=confHtml+'<div class="flip-scene" style="'+locked+'"><div class="flip-card'+(S.flipped?' flipped':'')+'" id="flip-c" onclick="'+clickFn+'"><div class="face face-front"><span class="face-status '+sc+'">'+sl+'</span><div class="face-lbl">Pytanie</div><div class="face-content" id="fc-f">'+renderMD(card.front)+'</div>'+(S.confidence&&!S.flipped?'<div class="face-hint">Dotknij aby odsłonic</div>':'')+(tags?'<div class="face-tags">'+tags+'</div>':'')+'</div><div class="face face-back"><div class="face-lbl">Odpowiedz</div><div class="face-content" id="fc-b">'+renderMD(card.back)+'</div></div></div></div>'+(S.flipped?'<div class="swipe-hint"><span style="color:var(--red)">&#8592; Nie pamieta</span><span style="color:var(--grn)">Umialem &#8594;</span></div><div style="padding:0 16px 6px;display:flex;gap:6px;flex-wrap:wrap"><button class="btn pur" onclick="showTransfer()" id="tbtn">&#128302; Pytanie praktyczne</button><button class="btn" onclick="showCompare()" id="cbtn">&#9878; Porownaj pojecia</button></div><div id="transfer-area"></div><div class="rating-row"><button class="rb bad" onclick="rate(1)"><span class="ico">&#10007;</span><span>Nie pamietalem</span><small>1</small></button><button class="rb ok" onclick="rate(3)"><span class="ico">&#12316;</span><span>Tak sobie</span><small>2</small></button><button class="rb good" onclick="rate(5)"><span class="ico">&#10003;</span><span>Umialem</span><small>3</small></button></div>':'<div style="height:10px"></div>');
  setupSwipe(document.getElementById('flip-c'));
...
function setupSwipe(el){
  ...
  el.addEventListener('touchend',e=>{
    ...
    if(Math.abs(dx)>Math.abs(dy)){if(dx>50)rate(5);else if(dx<-50)rate(1);}
...
function flipCard(){S.flipped=true;renderCard()}
